...
test("verified candidates are promoted while identity mismatches and duplicates are reported", async () => {
  const result = await verifyCandidates(candidates, {
    ...
    fetch: async (input) => {
      ...
      if (url.includes("api.lever.co")) return Response.json([{ id: "l1", text: "Engineer", hostedUrl: "https://jobs.lever.co/acme/l1", description: "Learn more at https://acme.test/careers" }]);
      if (url.includes("api.ashbyhq.com")) return Response.json({ jobs: [{ id: "a1", title: "Engineer", jobUrl: "https://jobs.ashbyhq.com/acme/a1", descriptionHtml: "Visit https://acme.test" }], apiVersion: "1" });
