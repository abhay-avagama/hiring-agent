  ...
  test("exposes only read-only search and get operations", async () => {
    ...
    const catalog: Catalog = {
      search: async (query) => {
        ...
        return [{ id: "lever:acme:1", company: "Acme", title: String(query.query), location: "Remote", remote: true, url: "https://example.test/1" }];
      },
      get: async () => null,
    };
    ...
    expect(await tools.call("search_jobs", { query: "Engineer", country: "IN", remote: true })).toEqual({
      jobs: [expect.objectContaining({ title: "Engineer" })],
    });
    expect(receivedCountry).toBe("IN");
