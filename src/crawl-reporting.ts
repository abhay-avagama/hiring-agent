...
export async function fetchSeedSnapshot(url: string, fetcher: Fetch = globalThis.fetch): Promise<JobSnapshot | null> {
  try {
    const response = await fetcher(endpoint(url, "v1/snapshot"), { signal: AbortSignal.timeout(120_000) });
