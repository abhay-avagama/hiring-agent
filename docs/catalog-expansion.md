...
- Stops immediately on HTTP 429/503 and persists a cooldown of at least 15 minutes or the longer `Retry-After`. HTTP 502/504 retry the same page at most twice, with 10s/20s backoff or a longer `Retry-After`; all attempts consume the request budget, and retry state survives restarts. After three gateway failures, stop with a minimum 15-minute cooldown. Waits longer than 60s yield to the operator. Other errors, including network exceptions, stop at the current checkpoint. HTTP failures retain the URL, status, allowlisted diagnostic headers (never cookies), and up to 2KB of response body; `lastFailure` remains historical if a later retry succeeds.
