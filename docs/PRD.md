...
The repository includes `data/companies-career-page.md` as an India-focused discovery asset. It contains company and career-page leads, not verified sources: most URLs are human-facing career pages, some are stale or third-party job-site links, and only a small number directly identify a supported ATS endpoint. A deterministic projection may treat a company label plus its company-owned HTTPS career hostname as `authoritative_dataset` identity input for Greenhouse and Workday only. Third-party links are excluded, every candidate still undergoes independent structured-provider and country-job verification, and Lever/Ashby still require provider-structured identity or a safely replayed company-owned redirect.
