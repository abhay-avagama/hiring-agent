...
The approved probe remains unexecuted: at most 20 authoritative non-ATS company identities, 200 detail pages, and 320 total requests; same-company HTTPS only; robots compliance; pinned public DNS; strict redirects, time, and body limits; no JavaScript or surrounding HTML extraction; and no writes to candidates, registries, catalog, or snapshots. It stops after the first ten companies on low yield or poor transport/schema health. The report must record the exact ordered identity sample and distinguish single-pass identifier presence from longitudinal stability. Production work remains blocked until the probe meets its employer/job/identifier/geography success gates.
